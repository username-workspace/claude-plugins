...
| [`mr-watchdog`](./plugins/mr-watchdog) | 2.2.1 | DevOps | Open a merge request, then forget it — a background watcher follows its CI and hands the verdict back to your session. |
| [`ship-when-done`](./plugins/ship-when-done) | 1.6.2 | DevOps | Commit at each milestone, push so nothing is lost, open the PR when it's actually done. |
